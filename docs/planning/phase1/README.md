...
Created: 18 September 2026 · Updated: 23 September 2026
Main: `9e4a0fa` · Base tag: `demo-opt-base`
...
| 6 | — | L2 `wp3-java-profiles` | — | **Open**, unstarted. WP3-5, WP3-6b, WP3-7b, WP3-9b, WP3-8 |
| — | [#59](https://github.com/cookedaryan/surge/pull/59) Refuse malformed and oversized V1 requests at the app layer | `ccr/main-request-guards` | — | **Open.** See §3 |
| — | [#38](https://github.com/cookedaryan/surge/pull/38) this record · [#40](https://github.com/cookedaryan/surge/pull/40) G0 decision · [#41](https://github.com/cookedaryan/surge/pull/41) refresh · [#47](https://github.com/cookedaryan/surge/pull/47) refresh | `docs/phase1-record` · `contracts/g0-claim` · `docs/phase1-refresh` · `docs/phase1-refresh-2` | `d9f5031` · `a0ecfc8` · `54cd560` · `37113f4` | **Merged.** Documentation branches; the ownership job does not run on them |

Every merged PR was rebased onto main before merging, per the queue rule, and CI was green on the rebased head. Tasks delivered: WP2-2, WP3-2, WP4-2, WP5-1 (L1); WP0B-7, WP0B-6, WP0B-11, WP5-2, WP5-3, WP5-4 (L3); WP1-1 … WP1-5, WP2-3 (L2); WP2-4 … WP2-8, WP3-4, WP3-3, WP3-1, WP3-7a, WP3-6a, WP3-9a (L3).

**Slots 1, 2, 4 and 5 are complete, and slot 6's L3 half is complete.** Typed identity runs end to end: Java sends it (#42), Python canonicalises it, carries it through ROW analysis and classifies constraints by it (#43), the canonical land and environment areas are computed from it (#46), and those areas now move a ranking a client can see (#48, #49). On top of that sits the whole profile mechanism: a versioned registry with the §7 structure (#50), fixed reference ranges that make the remove-a-loser invariant hold (#51), stable codes that fail closed instead of falling back to Balanced (#52), the recommendation itself driven by the resolved profile (#53), the `effective_profile` echo (#54), the C7 definition-hash handshake (#57) and producer conformance against the frozen fixtures (#58).

**What is left in Phase 1 is not L3 code.** Slot 3 waits on G2, slot 6's Java half is unstarted, and slot 1 still has an L2 straggler.

## 2. State of merged main

| Check | Result |
|---|---|
| Python suite on merged main | **1628 passed** at `9e4a0fa`, after slot 6's WP3-9a |
...
**Slot 6's Java half is the critical path.** `wp3-java-profiles` carries WP3-5, WP3-6b, WP3-7b, WP3-9b and WP3-8, and has no branch. Its shared oracles already exist on main: the C6 hash vector and the live endpoint for WP3-6b, and the frozen C1/C2/C11 fixtures that WP3-9a has now shown the Python side round-trips, for WP3-9b. **S2-1 requires slot 6 merged**, and FRZ-1 requires S2-1, so nothing in Stage 2 starts until this branch lands.
...
**[#59](https://github.com/cookedaryan/surge/pull/59) is open and needs a rebase.** It fixes two defects any client could trigger against `/api/v1/optimise`: a **500** where the contract promises a 422, because `json.loads` accepts the non-standard `Infinity` and `NaN` literals and FastAPI's default validation handler echoes the offending input back into a body that then cannot be serialised; and the C1 raw-body limit, which `_check_size` could never enforce because it measures the re-serialised payload — a 10.58 MB body measures 9.62 MB and was accepted. An ASGI middleware now stops an oversized request before anything parses it. It is a `ccr/` branch because `app/main.py` is in none of the ownership globs, not because it amends a contract: nothing frozen is touched and `export_contracts --check` reports `changed: []`. Its CI is green, but it branched from `50a8bcc` (#52) and so predates #53, #54, #57 and #58; its **1594 passed** is measured on that base and is not comparable with §2, and it must be rebased before merging. Two things in it are worth a second opinion: `VALUE_OUT_OF_RANGE` is doing duty as the general fallback, including for a missing field and for malformed JSON, because `ContractErrorCode` has no generic `INVALID_REQUEST` and adding one would be a C3 change; and the middleware is scoped to `/api/v1`, so **V2 still has no body-size limit**.

If #59 merges, `_check_size` becomes effectively unreachable over HTTP, since the canonical form is always smaller than the raw body. It is L3-owned and stays as the layer behind the middleware and for callers reaching `validate_request` directly. Removing it is not #59's call.
...
- #59's **1594 passed** is measured on `50a8bcc`, four merges behind §2's figure, and is not comparable with it.
...
- This is a running record and goes stale the moment the queue moves. It is accurate as of `9e4a0fa` on 23 September 2026.
