...
def test_repair_valid_immediately(monkeypatch, base_config):
    ...
    network = ProjectPNCNetwork(
        ...
        feeders=(
            PNCFeeder(
                ...
                segments=(
                    PNCSegment("S1", "F1", "SS", "W1", LineString([(0,0), (1,1)]), 1000.0, 1000.0, "substation_to_wtg"),
                ),
    ...
    monkeypatch.setattr("app.electrical.repair.size_cables_for_network", lambda *a, **kw: mock_sizing)
    ...
    monkeypatch.setattr("app.electrical.repair.run_load_flow", lambda *a, **kw: lf_result)
...
def test_overload_repair(monkeypatch, base_config):
    ...
    network = ProjectPNCNetwork(
        ...
        feeders=(
            PNCFeeder(
                ...
                segments=(
                    PNCSegment("S1", "F1", "SS", "W1", LineString([(0,0), (1,1)]), 1000.0, 1000.0, "substation_to_wtg"),
                    PNCSegment("S2", "F1", "W1", "W2", LineString([(1,1), (2,2)]), 1000.0, 1000.0, "wtg_to_wtg"),
                ),
    ...
    lf_result_1 = LoadFlowNetworkResult(
        ...
        buses=(),
        segments=(
            LoadFlowSegmentResult("S1", "F1", 0,0,0,0,0,0, 240.0, 240.0, 200.0, 120.0), # Needs >240A -> Cable-M (300A)
            LoadFlowSegmentResult("S2", "F1", 0,0,0,0,0,0, 310.0, 310.0, 200.0, 155.0), # Needs >310A -> Cable-L (500A)
        ),
        feeders=(),
        violations=(
            LoadFlowViolation(LoadFlowViolationCode.CABLE_OVERLOAD, "overload", segment_id="S1", measured_value=120.0),
            LoadFlowViolation(LoadFlowViolationCode.CABLE_OVERLOAD, "overload", segment_id="S2", measured_value=155.0),
        ),
    )
    ...
    lf_result_2 = LoadFlowNetworkResult(
        ...
        buses=(),
        segments=(
            LoadFlowSegmentResult("S1", "F1", 0,0,0,0,0,0, 240.0, 240.0, 300.0, 80.0),
            LoadFlowSegmentResult("S2", "F1", 0,0,0,0,0,0, 310.0, 310.0, 500.0, 62.0),
        ),
        feeders=(),
        violations=(),
    )
    ...
    monkeypatch.setattr("app.electrical.repair.run_load_flow", lambda *a, **kw: responses.pop(0))
...
def test_voltage_repair(monkeypatch, base_config):
    ...
    lf_result_1 = LoadFlowNetworkResult(
        ...
        segments=(
            LoadFlowSegmentResult("S1", "F1", 0,0,0,0,0,0, 100.0, 100.0, 200.0, 50.0),
            LoadFlowSegmentResult("S2", "F1", 0,0,0,0,0,0, 100.0, 100.0, 200.0, 50.0),
        ),
        feeders=(),
        violations=(
            LoadFlowViolation(LoadFlowViolationCode.BUS_UNDERVOLTAGE, "W1 undervolt", node_id="W1", measured_value=0.96, limit_value=0.95),
            LoadFlowViolation(LoadFlowViolationCode.BUS_UNDERVOLTAGE, "W2 undervolt", node_id="W2", measured_value=0.92, limit_value=0.95),
        ),
    )
    ...
    lf_result_2 = LoadFlowNetworkResult(
        ...
        segments=(
            LoadFlowSegmentResult("S1", "F1", 0,0,0,0,0,0, 100.0, 100.0, 300.0, 33.3),
            LoadFlowSegmentResult("S2", "F1", 0,0,0,0,0,0, 100.0, 100.0, 200.0, 50.0),
        ),
        feeders=(),
        violations=(),
    ...
    assert result.repair_log[0].original_cable_type_id == "Cable-S"
    assert result.repair_log[0].upgraded_cable_type_id == "Cable-M"
    assert result.repair_log[0].trigger_violation_type == str(LoadFlowViolationCode.BUS_UNDERVOLTAGE)
...
def test_voltage_upgrade_skips_worse_impedance(monkeypatch, base_config):
    ...
    lf_result_1 = LoadFlowNetworkResult(
        ...
        segments=(
            LoadFlowSegmentResult("S1", "F1", 0,0,0,0,0,0, 100.0, 100.0, 500.0, 20.0),
        ),
        feeders=(),
        violations=(
            LoadFlowViolation(LoadFlowViolationCode.BUS_UNDERVOLTAGE, "W1 undervolt", node_id="W1", measured_value=0.92, limit_value=0.95),
        ),
    )
    
    monkeypatch.setattr("app.electrical.repair.run_load_flow", lambda *a, **kw: lf_result_1)
    ...
    # Should not upgrade to Cable-Bad, since it has worse impedance, even if it has higher ampacity.
    # Therefore, exhausted.
