...
| `SYN-4-SPREAD-30.json` | Spread, south substation, hard and soft constraints | 30 | `SYNTHETIC` | `PYTHON_CONTRACT_VERIFIED` | `REAL_SOURCE`, `ROUND_TRIP_VERIFIED` |
...
the cable current limit in each request. `SYN-4` also uses a deliberately
high-resistance cable so the corpus includes successful and failed electrical
evaluations. They are synthetic Python contract fixtures and must not be
described as real-source or round-trip verified data.
