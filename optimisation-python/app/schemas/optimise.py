...
class ElectricalParams(BaseModel):
    feeder_capacity_mw: float = Field(
        default=20.0,
        gt=0,
        description="Maximum feeder capacity in MW",
    )

    max_voltage_drop_pct: float = Field(
        default=5.0,
        gt=0,
        le=100,
        description="Maximum allowable voltage drop percentage",
    )

    row_width_m: float = Field(
        default=18.0,
        gt=0,
...
class OptimisationRequest(BaseModel):
    request_id: str = Field(
        min_length=1,
        description="Correlation ID generated by the Java backend",
    )
