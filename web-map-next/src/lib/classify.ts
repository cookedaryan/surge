...
function folderSegmentsLeafFirst(folderPath: string | undefined): string[] {
  if (!folderPath) return [];
  const out = [];
  const parts = folderPath.split(/\s+\/\s+/).map((s: string) => s.trim()).reverse();
...
function statusFor(assetType: string, segments: string[]): string {
 ...
 */
export function classifyFeature(properties: Record<string, unknown> = {}) {
  const externalId = (properties.externalId || properties.name || properties.id || '').toString().trim();
  const folderPath = (properties.kmlFolderPath || properties.kmlFolder || '') as string;
...
export function normaliseType(value: unknown): string {
  if (!value) return ASSET_TYPES.UNKNOWN;
  const upper = value.toString().trim().toUpperCase().replace(/[\s-]/g, '_');
  if ((ASSET_TYPES as Record<string, string>)[upper]) return upper;
...
export function isOptimisable(status: string): boolean {
...
const LINE_FOLDER_KEYWORDS: Record<string, string[]> = {
...
const LINE_NAME_PATTERNS: [string, RegExp][] = [
...
const AREA_FOLDER_KEYWORDS: Record<string, string[]> = {
...
const AREA_NAME_PATTERNS: [string, RegExp][] = [
...
export function classifyLine(properties: Record<string, unknown> = {}) {
  const name = (properties.externalId || properties.name || '').toString().trim();
  const segments = folderSegmentsLeafFirst((properties.kmlFolderPath || properties.kmlFolder || '') as string);

  const explicit = (properties.lineType || '').toString().toUpperCase();
  if ((LINE_TYPES as Record<string, string>)[explicit]) return { lineType: explicit, matchedRule: 'EXPLICIT_PROPERTY' };
...
export function classifyPolygon(properties: Record<string, unknown> = {}) {
  const name = (properties.externalId || properties.name || '').toString().trim();
  const segments = folderSegmentsLeafFirst((properties.kmlFolderPath || properties.kmlFolder || '') as string);
...
export function classifyGeoJsonFeature(feature: import('geojson').Feature) {
